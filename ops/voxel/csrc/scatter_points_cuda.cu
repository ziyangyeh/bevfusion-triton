...
__global__ void
feats_reduce_kernel_fp32_accum(const T *feats, const int32_t *coors_map,
                               T_acc *reduced_feats,
                               const int num_input, const int num_feats,
                               const reduce_t reduce_type) {
  for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < num_input;
    ...
    const T *feats_offset = feats + x * num_feats;
    T_acc *reduced_feats_offset = reduced_feats + reduce_to * num_feats;
    if (reduce_type == reduce_t::MAX) {
      for (int i = 0; i < num_feats; i++) {
        reduceMax(&reduced_feats_offset[i], static_cast<T_acc>(feats_offset[i]));
      }
    } else {
      for (int i = 0; i < num_feats; i++) {
        reduceAdd(&reduced_feats_offset[i], static_cast<T_acc>(feats_offset[i]));
      }
    }
  }
}
...
std::vector<at::Tensor> dynamic_point_to_voxel_forward_gpu(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
    ...
    if constexpr (std::is_same_v<scalar_t, at::Half> ||
      ...
      auto reduced_feats_accum =
          at::empty({out_coors.size(0), num_feats},
                    feats.options().dtype(at::kFloat));
      if (reduce_type == reduce_t::MAX)
        reduced_feats_accum.fill_(-std::numeric_limits<float>::infinity());
      else
        reduced_feats_accum.fill_(0.0f);
      feats_reduce_kernel_fp32_accum<scalar_t, float><<<blocks, threads>>>(
          feats.data_ptr<scalar_t>(), coors_map.data_ptr<int32_t>(),
          reduced_feats_accum.data_ptr<float>(), num_input, num_feats,
          reduce_type);
      if (reduce_type == reduce_t::MEAN) {
        reduced_feats_accum /= reduce_count.unsqueeze(-1).to(reduced_feats_accum.dtype());
      }
      reduced_feats = reduced_feats_accum.to(feats.scalar_type());
    } else {
      if (reduce_type == reduce_t::MAX)
