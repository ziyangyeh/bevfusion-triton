...
_EXT_EXPORTED_FUNCTIONS = ['bev_pool_forward', 'bev_pool_backward']

ext_module = LazyInlineExtension(
	exported_names=_EXT_EXPORTED_FUNCTIONS,
	name='bev_pool_ext',
	source_dir=_CSRC_DIR,
	cpp_filenames=['bev_pool_cpu.cpp'],
	cuda_filenames=['bev_pool_cuda.cu'],
...
class QuickCumsumCuda(torch.autograd.Function):
	...
	def forward(ctx, x, geom_feats, ranks, B, D, H, W):
		...
		out = ext_module.bev_pool_forward(
			x,
			geom_feats,
			interval_lengths,
			interval_starts,
	...
	def backward(ctx, out_grad):
		...
		x_grad = ext_module.bev_pool_backward(
			out_grad,
			geom_feats,
			interval_lengths,
			interval_starts,
