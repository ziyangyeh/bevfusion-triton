...
class Custom3DDataset(Dataset):
	...
	def prepare_train_data(self, index):
		...
		if self.filter_empty_gt and (example is None or not self._has_valid_gt(example)):
			return None
		return example

	def _extract_gt_labels(self, example):
		labels = example.get('gt_labels_3d')
		if labels is None and isinstance(example.get('ann_info'), dict):
			labels = example['ann_info'].get('gt_labels_3d')
	...
	def _has_valid_gt(self, example):
		labels = self._extract_gt_labels(example)
		if labels is None:
			return False
		if isinstance(labels, np.ndarray):
			return bool((labels != -1).any())
		if torch.is_tensor(labels):
			return bool((labels != -1).any().item())
	...
	def get_classes(cls, classes=None):
		...
		elif isinstance(classes, Sequence) and not isinstance(classes, str):
