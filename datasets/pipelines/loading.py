...
from core.points import BasePoints, RadarPoints, get_points_type
from core.mask import BitmapMasks, PolygonMasks
...
class LoadAnnotations:
	...
	def _load_semantic_seg(self, results):
		...
		with open(filename, 'rb') as f:
			img_bytes = f.read()
		results['gt_semantic_seg'] = imfrombytes(img_bytes, flag='unchanged').squeeze()
...
class LoadRadarPointsMultiSweeps(object):
	...
	def _load_points(self, pts_filename):
		...
		radar_obj = RadarPointCloud.from_file(
			pts_filename, invalid_states, dynprop_states, ambig_states
		)
		...
		for key, sweeps in radars_dict.items():
			...
			for idx in idxes:
				...
				points_sweep_ = np.concatenate(
					[points_sweep[:, :6], velo, velo_comp, points_sweep[:, 10:], time_diff], axis=1
				)
