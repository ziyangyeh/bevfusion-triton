...
def imresize(img, size, interpolation='bilinear', out=None):
...
def imrotate(
	img, angle, center=None, scale=1.0, border_value=0, interpolation='bilinear', auto_bound=False
):
	"""Rotate an image.
...
class BitmapMasks(BaseInstanceMasks):
	...
	Example:
	    ...
	    >>> new = self.crop_and_resize(bboxes, out_shape, inds, device, interpolation)
	...
	def rescale(self, scale, interpolation='nearest'):
		...
		if len(self.masks) == 0:
			...
			rescaled_masks = np.stack(
				[imrescale(mask, scale, interpolation=interpolation) for mask in self.masks]
			)
	...
	def resize(self, out_shape, interpolation='nearest'):
		...
		if len(self.masks) == 0:
			...
			resized_masks = np.stack(
				[imresize(mask, out_shape, interpolation=interpolation) for mask in self.masks]
			)
