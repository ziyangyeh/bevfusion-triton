...
import cv2
import numpy as np
from datasets.nuscenes_dataset_legacy import NuScenesDataset
...
from core.bbox.box_np_ops import points_cam2img
...
def create_nuscenes_infos(
	root_path, info_prefix, version='v1.0-trainval', max_sweeps=10, max_radar_sweeps=10
):
...
def _fill_trainval_infos(
	nusc, train_scenes, val_scenes, test=False, max_sweeps=10, max_radar_sweeps=10
):
			...
			while len(sweeps) < max_radar_sweeps:
				if not radar_rec['prev'] == '':
					...
					radar_info = obtain_sensor2top(
						nusc, radar_token, l2e_t, l2e_r_mat, e2g_t, e2g_r_mat, radar_name
					)
...
def export_2d_annotation(root_path, info_path, version, mono3d=True):
	...
	with open(f'{json_prefix}.coco.json', 'w') as f:
