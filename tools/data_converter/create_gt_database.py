...
from core.bbox import box_np_ops as box_np_ops
...
def create_groundtruth_database(
	...
	pipeline = [
		...
		LoadPointsFromMultiSweeps(
			sweeps_num=10, use_dim=[0, 1, 2, 3, 4], pad_empty_sweeps=True, remove_close=True
		),
		LoadAnnotations3D(with_bbox_3d=True, with_label_3d=True),
	]
	dataset = NuScenesDataset(dataset_root=data_path, ann_file=info_path, pipeline=pipeline)
	...
	for j in tqdm(list(range(len(dataset)))):
		...
		for i in range(num_obj):
			...
			if with_mask:
				...
				imwrite(object_img_patches[i], img_patch_path)
				imwrite(object_masks[i], mask_patch_path)
